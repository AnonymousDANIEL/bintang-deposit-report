...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            site_api_url=_env(
                "SITE_API_URL",
                "https://90018.gawlt.com/api/v1/index.php",
            ),
            ...
            target_count=_int("TARGET_COUNT", 3500),
            target_deposit=Decimal(_env("TARGET_DEPOSIT", "60000")),
            report_timezone=_env("REPORT_TIMEZONE", "Asia/Kuala_Lumpur"),
            report_line_style=style,
            state_path=_env("STATE_PATH", "/data/bintang44_state.json"),
            ...
            session_guard_seconds=max(15, _int("SESSION_GUARD_SECONDS", 30)),
            auth_relogin_retries=max(1, _int("AUTH_RELOGIN_RETRIES", 5)),
            # Exact-hour build: run immediately at HH:00; do not wait +1 minute.
