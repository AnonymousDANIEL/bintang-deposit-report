...
def build_message(snapshot: ReportSnapshot, cfg: Config) -> str:
    """
    Plain Telegram text only: no <pre>/<code>, so no Telegram Copy button.
    Fixed order is always 0100..2300,0000.

    A closed slot shows its real hour values and real cumulative TOTAL.
    A future slot shows zero for Count, Amount and TOTAL until that slot closes.
    """
    lines = [
        "Bintang44 Deposit Report",
        "",
