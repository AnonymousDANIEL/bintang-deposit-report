...
from bintang44 import BintangClient, Totals
...
log = logging.getLogger("bintang44-report")
...
def collect_snapshot(cfg: Config, client: BintangClient, now: datetime) -> ReportSnapshot:
    tz = ZoneInfo(cfg.report_timezone)
    report_date, closed_end = closed_report_window(now, tz)
...
def run_once(cfg: Config, client: BintangClient | None = None) -> str:
    state = StateStore(cfg.state_path)
    bot = TelegramBot(cfg, state)
    client = client or BintangClient(cfg)
...
def _send_error_once(cfg: Config, exc: Exception) -> None:
    ...
    try:
        bot.alert_once(
            fingerprint,
            "Bintang44 Deposit Report ERROR\n\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Bintang44 hourly Telegram deposit report")
    ...
    try:
        ...
        if args.test_telegram:
            msg_id = bot.send_message(cfg.telegram_chat_id, "Bintang44 Telegram Bot ✅ Connected", silent=False)
